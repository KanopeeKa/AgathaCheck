...
export function GuardianOperationsMobileMistedPlumState() {
  ...
  return <main className="min-h-[100dvh] bg-[#f4f2ed] font-sans text-[#332434]">
    <div className="mx-auto w-full max-w-[430px] overflow-hidden bg-[#f4f2ed] shadow-[0_0_50px_rgba(72,38,64,.08)]">
      ...
      {state !== 'today' ? <EmptyTreatment state={state} onBack={() => setState('today')} onState={(next) => setState(next)} /> : <div className="phone-reveal space-y-3 px-4 pb-24 pt-4">
        ...
        <section className="overflow-hidden rounded-[22px] bg-[#e8e1e6]"><div className="flex items-center justify-between border-b border-[#d3c7d1] px-4 py-3"><div><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#765b70]">Care</p><h2 className="text-[16px] font-bold">Due this week <span className="ml-1 rounded-full bg-[#753463] px-2 py-0.5 text-[10px] text-white">3</span></h2></div><button onClick={() => setState('care')} className="text-[11px] font-semibold text-[#6e2e61]">View all <ChevronRight size={13} className="inline"/></button></div>
          {[['Bailey','Heartworm Preventive','Due today'],['Luna','Rabies Vaccine','Due in 3 days'],['Bailey','Nail Trim','2 days overdue']].map((row, i) => <div key={i} className="flex items-center gap-3 border-b border-[#d3c7d1] px-4 py-3"><div className="grid h-9 w-9 place-items-center rounded-xl bg-[#f5f1f3] text-[#735b6e]">{i === 2 ? <Heart size={17}/> : <CalendarDays size={17}/>}</div><div className="min-w-0 flex-1"><p className="text-[12px] font-bold">{row[0]}</p><p className="text-[11px] text-[#665d66]">{row[1]}</p><p className={`text-[10px] ${i === 2 ? 'text-[#bd4f5a]' : 'text-[#756972]'}`}>{row[2]}</p></div><button onClick={() => toggle(i)} className={`rounded-xl px-3 py-2 text-[10px] font-bold ${done.includes(i) ? 'bg-[#2c807b] text-white' : 'bg-[#763463] text-white'}`}>{done.includes(i) ? <Check size={14}/> : 'Mark done'}</button><ChevronRight size={15} className="text-[#6e2e61]"/></div>)}
...
export default GuardianOperationsMobileMistedPlumState;