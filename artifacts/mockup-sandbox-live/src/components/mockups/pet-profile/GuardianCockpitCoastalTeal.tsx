    ...
    <main className="gc-shell">
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600;700&family=Fraunces:opsz,wght@9..144,500&display=swap');
         .gc-shell { min-height:100vh; background:#edf5f3; color:#183b40; font-family:'DM Sans',sans-serif; display:flex; }
         .gc-sidebar { width:226px; flex:0 0 226px; background:#153c43; color:#edf8f5; padding:25px 16px 20px; display:flex; flex-direction:column; }
        .gc-brand { display:flex; align-items:center; gap:10px; padding:0 10px 32px; font-weight:700; letter-spacing:-.03em; }
         .gc-brand-mark { width:30px;height:30px;border-radius:10px;background:#e1a17e;display:grid;place-items:center;color:#153c43; }
         .gc-nav-label { color:#83a9aa; font-size:10px; letter-spacing:.14em; text-transform:uppercase; margin:8px 12px 10px; }
         .gc-nav { display:flex; align-items:center; gap:11px; width:100%; border:0; background:none; color:#a7c3c1; padding:11px 12px; border-radius:11px; text-align:left; font:500 13px 'DM Sans'; cursor:pointer; }
         .gc-nav.active { color:#f6fcfa; background:#276169; }
         .gc-nav:hover { background:#1d535b; color:#f6fcfa; }
         .gc-sidebar-foot { margin-top:auto; border-top:1px solid #2a5960; padding-top:18px; }
        .gc-main { min-width:0; flex:1; }
         .gc-topbar { height:78px; background:#f5faf8; border-bottom:1px solid #d8e8e4; display:flex; align-items:center; justify-content:space-between; padding:0 clamp(20px,4vw,56px); }
         .gc-menu { display:none; border:0;background:none;color:#153c43; }
         .gc-crumb { font-size:12px; color:#718c8d; display:flex; gap:8px; align-items:center; }
         .gc-crumb strong { color:#214b50; font-weight:600; }
        .gc-top-actions { display:flex; gap:12px; align-items:center; }
         .gc-icon-button { width:38px;height:38px;border:1px solid #d6e5e1;background:#fdfefd;border-radius:11px;display:grid;place-items:center;color:#527477;cursor:pointer; }
        ...
        .gc-welcome { display:flex; align-items:flex-end; justify-content:space-between; gap:20px; margin-bottom:32px; }
         .gc-eyebrow { color:#4d8583; text-transform:uppercase; letter-spacing:.15em; font-size:10px; font-weight:700; margin-bottom:10px; }
         .gc-h1 { font-size:clamp(28px,4vw,40px);line-height:1.05;letter-spacing:-.055em;margin:0;color:#183b40; }
         .gc-h1 em { font-family:Fraunces,serif;font-weight:500;color:#287f7c;font-style:normal; }
         .gc-date { color:#6d8585;font-size:13px;text-align:right;line-height:1.5; }
         .gc-date b { color:#315c60; display:block; font-size:14px; }
        .gc-grid { display:grid; grid-template-columns:minmax(0,1.45fr) minmax(260px,.8fr); gap:22px; }
         .gc-card { background:#fbfefd;border:1px solid #d8e8e4;border-radius:18px;box-shadow:0 8px 24px rgba(20,75,78,.06); }
         .gc-hero { padding:24px; background:#dcefeb; border-color:#c5e0da; position:relative; overflow:hidden; }
         .gc-hero:after { content:'';position:absolute;right:-55px;top:-75px;width:220px;height:220px;border-radius:50%;border:1px solid rgba(40,127,124,.18);box-shadow:0 0 0 22px rgba(40,127,124,.05),0 0 0 44px rgba(40,127,124,.035); }
        .gc-pet-select { position:relative;z-index:2;display:flex;align-items:center;justify-content:space-between; }
         .gc-pet-button { display:flex;align-items:center;gap:12px;background:none;border:0;padding:0;cursor:pointer;color:#183b40;text-align:left; }
        ...
        .gc-pet-name { font-weight:700;font-size:20px;letter-spacing:-.04em;display:block; }
         .gc-pet-meta { color:#557a7b;font-size:12px;display:block;margin-top:2px; }
         .gc-pet-menu { position:absolute;right:24px;top:84px;background:#fbfefd;border:1px solid #cfe2de;border-radius:12px;padding:5px;box-shadow:0 12px 30px #214b5020;z-index:5;min-width:150px; }
        .gc-pet-menu button { width:100%;border:0;background:none;padding:9px;text-align:left;border-radius:8px;color:#554a50;cursor:pointer;font:500 12px 'DM Sans'; }
         .gc-pet-menu button:hover { background:#e5f2ef; }
        .gc-health { margin-top:27px;display:flex;align-items:flex-end;justify-content:space-between;position:relative;z-index:1; }
         .gc-health-label { color:#4e7e7d;font-size:12px;margin-bottom:7px; }
         .gc-health-score { font-size:37px;letter-spacing:-.06em;font-weight:700;color:#1f4c50; }
         .gc-health-score small { font-size:14px;color:#6b8b8b;font-weight:500;letter-spacing:0; }
         .gc-ring { width:91px;height:91px;border-radius:50%;display:grid;place-items:center;background:conic-gradient(#287f7c 0 82%, #b9d9d4 82% 100%); position:relative; }
         .gc-ring:after { content:'';position:absolute;inset:8px;background:#dcefeb;border-radius:50%; }
         .gc-ring span { position:relative;z-index:1;font-size:18px;font-weight:700;color:#216866; }
         .gc-progress { height:5px;background:#b9d9d4;border-radius:9px;margin-top:17px;overflow:hidden;position:relative;z-index:1; }
         .gc-progress i { display:block;width:82%;height:100%;background:#287f7c;border-radius:9px; }
        ...
        .gc-card-title { font-size:15px;font-weight:700;letter-spacing:-.02em; }
         .gc-card-link { color:#287f7c;border:0;background:none;font:600 11px 'DM Sans';cursor:pointer; }
        .gc-tasks { padding:0 12px 10px; }
         .gc-task { display:grid;grid-template-columns:50px 35px 1fr 28px;align-items:center;gap:10px;padding:13px 11px;border-top:1px solid #e2eeeb; }
        ...
        .gc-medicine { background:#f3e6e2;color:#b06f69; } .gc-care { background:#e4f0eb;color:#438d74; } .gc-walk { background:#e7edf4;color:#62809e; }
         .gc-task-title { font-size:13px;font-weight:600;color:#285055;display:block; }
         .gc-task-detail { font-size:11px;color:#789091;margin-top:3px;display:block; }
        .gc-task.is-done .gc-task-title { text-decoration:line-through;color:#aaa19d; }
         .gc-check { width:23px;height:23px;border:1px solid #c6dcd7;border-radius:50%;background:#fdfefd;display:grid;place-items:center;color:transparent;cursor:pointer; }
         .gc-check.checked { background:#287f7c;border-color:#287f7c;color:#fff; }
        .gc-stat-card { padding:22px 23px; }
        .gc-stat-row { display:flex;justify-content:space-between;align-items:center; }
         .gc-stat-main { font-size:30px;font-weight:700;letter-spacing:-.05em;color:#1f4c50; }
         .gc-stat-main small { font-size:12px;font-weight:500;color:#789091;letter-spacing:0; }
         .gc-stat-note { font-size:11px;color:#71898a;margin-top:5px; }
         .gc-stat-icon { width:37px;height:37px;border-radius:12px;background:#e0f1ed;color:#287f7c;display:grid;place-items:center; }
        .gc-mini-chart { display:flex;align-items:flex-end;gap:7px;height:53px;margin-top:22px; }
         .gc-mini-chart i { flex:1;background:#b9d9d4;border-radius:5px 5px 2px 2px;min-height:15px; } .gc-mini-chart i:nth-child(4),.gc-mini-chart i:last-child { background:#287f7c; }
        .gc-side-stack { display:flex;flex-direction:column;gap:22px; }
        .gc-next { padding:22px 23px; }
         .gc-next-box { background:#e8f3f0;border-radius:12px;padding:14px;margin-top:14px;display:flex;gap:11px;align-items:center; }
        .gc-next-box strong { font-size:12px;display:block; } .gc-next-box span { font-size:11px;color:#918580;display:block;margin-top:3px; }
         .gc-add { width:100%;border:1px dashed #a9cbc5;border-radius:11px;background:#f3faf8;color:#287f7c;font:600 12px 'DM Sans';padding:12px;cursor:pointer;margin-top:12px; }
         .gc-composer { border-top:1px solid #dcebe7;padding:14px 12px;display:flex;gap:8px;align-items:center; } .gc-composer input { flex:1;border:1px solid #c7ddd8;border-radius:9px;padding:10px;font:12px 'DM Sans';outline:none; } .gc-composer button { border:0;border-radius:9px;background:#287f7c;color:#fff;padding:10px 14px;font:600 12px 'DM Sans';cursor:pointer; }
         .gc-toast { position:fixed;bottom:22px;left:50%;transform:translateX(-50%);background:#153c43;color:#effaf7;padding:11px 15px;border-radius:10px;font-size:12px;box-shadow:0 8px 22px #153c4333;z-index:20; }
      ...
      <aside className="gc-sidebar">
        ...
        <div className="gc-sidebar-foot">
          <div style={{display:'flex',alignItems:'center',gap:10,padding:'0 9px'}}>
             <div className="gc-avatar" style={{background:'#397b7d'}}>JM</div>
        ...
        <header className="gc-topbar">
          ...
          <div className="gc-top-actions">
            <button className="gc-icon-button" onClick={() => toast('No new notifications')}><Bell size={17}/></button>
             <button className="gc-avatar" style={{background:'#397b7d',border:0,cursor:'pointer'}} onClick={() => toast('Profile menu opened')}>JM</button>
          </div>
        </header>
          ...
          <div className="gc-grid">
              ...
              <article className="gc-card gc-hero">
                <div className="gc-pet-select">
                  <button className="gc-pet-button" onClick={() => setShowPetMenu(!showPetMenu)}>
                   ...
                   <span><span className="gc-pet-name">{pet.name} <ChevronDown size={15} style={{verticalAlign:'-2px'}}/></span><span className="gc-pet-meta">{pet.species} · <span style={{color:'#287f7c'}}>● {pet.status}</span></span></span>
                  </button>
            ...
            <div className="gc-side-stack">
              ...
              <article className="gc-card gc-next"><div className="gc-card-head" style={{padding:0}}><div><div className="gc-eyebrow" style={{marginBottom:5}}>Next appointment</div><div className="gc-card-title">Annual wellness exam</div></div><Stethoscope size={20} color="#755b68"/></div><div className="gc-next-box"><CalendarDays size={17} color="#755b68"/><div><strong>Thursday, 23 May</strong><span>10:30 · Cedar Grove Vet</span></div></div><button className="gc-add" onClick={() => toast('Appointment details opened')}>View appointment <ChevronRight size={13} style={{verticalAlign:'-2px'}}/></button></article>
                 <article className="gc-card" style={{padding:'20px 22px',background:'#e2f2ee',borderColor:'#cae4dd'}}><div style={{display:'flex',gap:11,alignItems:'flex-start'}}><ShieldCheck size={20} color="#287f7c"/><div><div style={{fontSize:13,fontWeight:700,color:'#285e5b'}}>Everything looks on track</div><div style={{fontSize:11,color:'#668b86',lineHeight:1.5,marginTop:5}}>Miso's routine is 3 days stronger than last week.</div></div></div></article>
            </div>
          </div>
        </div>
      </section>
