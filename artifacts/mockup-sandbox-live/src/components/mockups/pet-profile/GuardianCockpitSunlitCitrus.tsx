...
export default function GuardianCockpitSunlitCitrus() {
  ...
  return (
    <main className="gc-shell">
      <style>{`
        ...
        .gc-shell { min-height:100vh; background:#fff8e9; color:#283b36; font-family:'DM Sans',sans-serif; display:flex; }
        .gc-sidebar { width:226px; flex:0 0 226px; background:#243b35; color:#fff8e9; padding:25px 16px 20px; display:flex; flex-direction:column; }
        .gc-brand { display:flex; align-items:center; gap:10px; padding:0 10px 32px; font-weight:700; letter-spacing:-.03em; }
        .gc-brand-mark { width:30px;height:30px;border-radius:10px;background:#f4ad3d;display:grid;place-items:center;color:#243b35; }
        .gc-nav-label { color:#a9c0b5; font-size:10px; letter-spacing:.14em; text-transform:uppercase; margin:8px 12px 10px; }
        .gc-nav { display:flex; align-items:center; gap:11px; width:100%; border:0; background:none; color:#c5d7cd; padding:11px 12px; border-radius:11px; text-align:left; font:500 13px 'DM Sans'; cursor:pointer; }
        .gc-nav.active { color:#243b35; background:#f4ad3d; }
        .gc-nav:hover { background:#315047; color:#fff8e9; }
        .gc-sidebar-foot { margin-top:auto; border-top:1px solid #3b5a50; padding-top:18px; }
        .gc-main { min-width:0; flex:1; }
        .gc-topbar { height:78px; background:#fffdf6; border-bottom:1px solid #eadfc8; display:flex; align-items:center; justify-content:space-between; padding:0 clamp(20px,4vw,56px); }
        .gc-menu { display:none; border:0;background:none;color:#243b35; }
        .gc-crumb { font-size:12px; color:#87958c; display:flex; gap:8px; align-items:center; }
        .gc-crumb strong { color:#324b42; font-weight:600; }
        .gc-top-actions { display:flex; gap:12px; align-items:center; }
        .gc-icon-button { width:38px;height:38px;border:1px solid #e4d9c2;background:#fffdf6;border-radius:11px;display:grid;place-items:center;color:#61736b;cursor:pointer; }
        ...
        .gc-eyebrow { color:#b66c32; text-transform:uppercase; letter-spacing:.15em; font-size:10px; font-weight:700; margin-bottom:10px; }
        .gc-h1 { font-size:clamp(28px,4vw,40px);line-height:1.05;letter-spacing:-.055em;margin:0;color:#243b35; }
        .gc-h1 em { font-family:Fraunces,serif;font-weight:500;color:#d87550;font-style:normal; }
        .gc-date { color:#71827a;font-size:13px;text-align:right;line-height:1.5; }
        .gc-date b { color:#39534a; display:block; font-size:14px; }
        .gc-grid { display:grid; grid-template-columns:minmax(0,1.45fr) minmax(260px,.8fr); gap:22px; }
        .gc-card { background:#fffdf6;border:1px solid #eadfc8;border-radius:18px;box-shadow:0 8px 24px rgba(93,76,36,.07); }
        .gc-hero { padding:24px; background:#fff0c9; border-color:#f0d99b; position:relative; overflow:hidden; }
        .gc-hero:after { content:'';position:absolute;right:-55px;top:-75px;width:220px;height:220px;border-radius:50%;border:1px solid rgba(216,117,80,.18);box-shadow:0 0 0 22px rgba(216,117,80,.05),0 0 0 44px rgba(244,173,61,.06); }
        ...
        .gc-pet-meta { color:#6e7d73;font-size:12px;display:block;margin-top:2px; }
        .gc-pet-menu { position:absolute;right:24px;top:84px;background:#fffdf6;border:1px solid #e4d6b9;border-radius:12px;padding:5px;box-shadow:0 12px 30px #6b5a321a;z-index:5;min-width:150px; }
        ...
        .gc-health-label { color:#9c6b38;font-size:12px;margin-bottom:7px; }
        .gc-health-score { font-size:37px;letter-spacing:-.06em;font-weight:700;color:#294139; }
        .gc-health-score small { font-size:14px;color:#8b8d72;font-weight:500;letter-spacing:0; }
        .gc-ring { width:91px;height:91px;border-radius:50%;display:grid;place-items:center;background:conic-gradient(#d87550 0 82%, #f1d99d 82% 100%); position:relative; }
        .gc-ring:after { content:'';position:absolute;inset:8px;background:#fff0c9;border-radius:50%; }
        .gc-ring span { position:relative;z-index:1;font-size:18px;font-weight:700;color:#b45c3e; }
        .gc-progress { height:5px;background:#efd99f;border-radius:9px;margin-top:17px;overflow:hidden;position:relative;z-index:1; }
        .gc-progress i { display:block;width:82%;height:100%;background:#d87550;border-radius:9px; }
        ...
        .gc-card-link { color:#c36343;border:0;background:none;font:600 11px 'DM Sans';cursor:pointer; }
        .gc-tasks { padding:0 12px 10px; }
        .gc-task { display:grid;grid-template-columns:50px 35px 1fr 28px;align-items:center;gap:10px;padding:13px 11px;border-top:1px solid #f0e8d8; }
        .gc-task-time { font-size:11px;color:#8a968e;font-variant-numeric:tabular-nums; }
        .gc-icon { width:33px;height:33px;border-radius:10px;display:grid;place-items:center; }
        .gc-medicine { background:#fbe3d9;color:#c15e43; } .gc-care { background:#e2f0dc;color:#478566; } .gc-walk { background:#e5eee9;color:#4f7d73; }
        .gc-task-title { font-size:13px;font-weight:600;color:#344a41;display:block; }
        .gc-task-detail { font-size:11px;color:#8d988f;margin-top:3px;display:block; }
        .gc-task.is-done .gc-task-title { text-decoration:line-through;color:#aaa19d; }
        .gc-check { width:23px;height:23px;border:1px solid #d8d4bb;border-radius:50%;background:#fffdf6;display:grid;place-items:center;color:transparent;cursor:pointer; }
        .gc-check.checked { background:#4b8a68;border-color:#4b8a68;color:#fffdf6; }
        .gc-stat-card { padding:22px 23px; }
        .gc-stat-row { display:flex;justify-content:space-between;align-items:center; }
        .gc-stat-main { font-size:30px;font-weight:700;letter-spacing:-.05em;color:#294139; }
        .gc-stat-main small { font-size:12px;font-weight:500;color:#89968e;letter-spacing:0; }
        .gc-stat-note { font-size:11px;color:#7f9086;margin-top:5px; }
        .gc-stat-icon { width:37px;height:37px;border-radius:12px;background:#e2f0dc;color:#478566;display:grid;place-items:center; }
        .gc-mini-chart { display:flex;align-items:flex-end;gap:7px;height:53px;margin-top:22px; }
        .gc-mini-chart i { flex:1;background:#d4e5cd;border-radius:5px 5px 2px 2px;min-height:15px; } .gc-mini-chart i:nth-child(4),.gc-mini-chart i:last-child { background:#4b8a68; }
        .gc-side-stack { display:flex;flex-direction:column;gap:22px; }
        .gc-next { padding:22px 23px; }
        .gc-next-box { background:#fff3d9;border-radius:12px;padding:14px;margin-top:14px;display:flex;gap:11px;align-items:center; }
        .gc-next-box strong { font-size:12px;display:block; } .gc-next-box span { font-size:11px;color:#819087;display:block;margin-top:3px; }
        .gc-add { width:100%;border:1px dashed #d8b66f;border-radius:11px;background:#fffaf0;color:#bf6544;font:600 12px 'DM Sans';padding:12px;cursor:pointer;margin-top:12px; }
        .gc-composer { border-top:1px solid #eee4d2;padding:14px 12px;display:flex;gap:8px;align-items:center; } .gc-composer input { flex:1;border:1px solid #ded4bc;border-radius:9px;padding:10px;font:12px 'DM Sans';outline:none; } .gc-composer button { border:0;border-radius:9px;background:#d87550;color:#fffdf6;padding:10px 14px;font:600 12px 'DM Sans';cursor:pointer; }
        .gc-toast { position:fixed;bottom:22px;left:50%;transform:translateX(-50%);background:#243b35;color:#fff8e9;padding:11px 15px;border-radius:10px;font-size:12px;box-shadow:0 8px 22px #243b3533;z-index:20; }
      ...
      <aside className="gc-sidebar">
        ...
        <div className="gc-sidebar-foot">
          <div style={{display:'flex',alignItems:'center',gap:10,padding:'0 9px'}}>
             <div className="gc-avatar" style={{background:'#d87550'}}>JM</div>
             <div><div style={{fontSize:12,fontWeight:600}}>Jordan Miller</div><div style={{fontSize:10,color:'#a9c0b5',marginTop:2}}>Guardian account</div></div>
          </div>
        </div>
      </aside>
      ...
      <section className="gc-main">
        <header className="gc-topbar">
          ...
          <div className="gc-top-actions">
            ...
            <button className="gc-avatar" style={{background:'#d87550',border:0,cursor:'pointer'}} onClick={() => toast('Profile menu opened')}>JM</button>
          </div>
        </header>
        ...
        <div className="gc-content">
          ...
          <div className="gc-grid">
                ...
                <div className="gc-pet-select">
                  <button className="gc-pet-button" onClick={() => setShowPetMenu(!showPetMenu)}>
                    <span className="gc-pet-photo" style={{background:pet.tone}}>{pet.initials}</span>
                     <span><span className="gc-pet-name">{pet.name} <ChevronDown size={15} style={{verticalAlign:'-2px'}}/></span><span className="gc-pet-meta">{pet.species} · <span style={{color:'#4b8a68'}}>● {pet.status}</span></span></span>
                  </button>
                   <button className="gc-icon-button" style={{background:'#fff4dc',borderColor:'#ebd49b'}} onClick={() => toast('Pet profile opened')}><MoreHorizontal size={18}/></button>
                </div>
            ...
            <div className="gc-side-stack">
              ...
              <article className="gc-card gc-next"><div className="gc-card-head" style={{padding:0}}><div><div className="gc-eyebrow" style={{marginBottom:5}}>Next appointment</div><div className="gc-card-title">Annual wellness exam</div></div><Stethoscope size={20} color="#d87550"/></div><div className="gc-next-box"><CalendarDays size={17} color="#d87550"/><div><strong>Thursday, 23 May</strong><span>10:30 · Cedar Grove Vet</span></div></div><button className="gc-add" onClick={() => toast('Appointment details opened')}>View appointment <ChevronRight size={13} style={{verticalAlign:'-2px'}}/></button></article>
               <article className="gc-card" style={{padding:'20px 22px',background:'#e6f1df',borderColor:'#cfe1c5'}}><div style={{display:'flex',gap:11,alignItems:'flex-start'}}><ShieldCheck size={20} color="#4b8a68"/><div><div style={{fontSize:13,fontWeight:700,color:'#356247'}}>Everything looks on track</div><div style={{fontSize:11,color:'#6f8c79',lineHeight:1.5,marginTop:5}}>Miso's routine is 3 days stronger than last week.</div></div></div></article>
            </div>
          </div>
        </div>
      </section>
       {notice && <div className="gc-toast"><Sparkles size={13} style={{verticalAlign:'-2px',marginRight:7,color:'#f4ad3d'}}/>{notice}</div>}
    </main>
  );
}