...
When `./scripts/pre-push-changed.sh` detects edits under `flutter_app/lib/features/*/presentation/**`, theme, or router files, it prints a **UI touch reminder**. Complete **`.cursor/agent-kernel/protocols/accessibility.md` §Quick pass** before opening or updating the PR. Escalate to **`/ui-design-deep`** for theme, landing/auth, or multi-screen work.
