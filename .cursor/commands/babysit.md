...
**Pre-PR review:** If the PR is not open yet, complete mandatory pre-PR critical self-review first (`.cursor/rules/pr-hygiene.mdc`).
...
4. **Wait for Bugbot (mandatory):** `node scripts/babysit_pr_reviews.js wait --pr <url>` — polls up to 15 minutes. Do not block on Copilot timeout when Bugbot is done or unavailable.
   - **Bugbot timeout:** comment and halt. **Copilot unavailable:** proceed after Bugbot; note on PR only when Copilot never posted.
5. **Collect threads:** `node scripts/babysit_pr_reviews.js collect --pr <url>` — triage **every** unresolved thread in the JSON output (Bugbot, Copilot, humans). Never skip Copilot threads because Bugbot hit a usage limit.
6. **Comments:** Apply triage buckets to collected threads. Validate findings — fix valid issues; explain false positives. Do not re-run full adversarial review in agent turns (slim babysit).
7. **CI:** Fix failures caused by this PR's scope. Never weaken CI gates to pass. If failure seems unrelated, run `./scripts/babysit_sync_base.sh --pr <url> --push` first — do not wait for a red CI run to discover the branch is behind.
8. **Verify locally:** `./scripts/pre-push-changed.sh` for scoped fix; `./scripts/pre-push.sh` before final push when near merge.
9. **Push** scoped fixes and re-watch CI until mergeable + green + comments triaged. Run `./scripts/babysit_sync_base.sh --pr <url>` before each push.
