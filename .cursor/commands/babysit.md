...
4. **Wait for Bugbot (mandatory):** Poll until **Bugbot** completes or the budget expires. Copilot is optional; do not halt on Copilot timeout when Bugbot is done.
   ...
   - Poll every 30–60s for up to **15 minutes**.
   - **Bugbot timeout:** comment and halt. **Copilot unavailable:** proceed after Bugbot; note on PR.
5. **Comments:** Triage active unresolved threads (Bugbot, Copilot if any, humans). Filter resolved threads. Validate findings — fix valid issues; explain false positives. Do not re-run full adversarial review in agent turns (slim babysit).
