...
6. **Issue hygiene** — control issue: comment milestones; `start-work` + `busy` on session start; close on complete (`complete-plan --write`). Project board columns are human/Actions — agents do not update them. UAT babysit sub-agent spawns after merge while next phase continues (babysit-plus §8).
