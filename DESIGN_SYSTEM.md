...
| `neutral-100` | `#F4F2ED` | warm page background |
...
```css
:root {
  --color-bg-primary: #F4F2ED;       /* Main app canvas */
...
| Token | Hex | Usage |
|---|---:|---|
| `landing-paper` | `#F4F2ED` | warm public canvas |
...
Root surfaces use a hamburger. Sub-screens use a back control. The Guardian
mobile care desk may additionally expose the documented five-destination bar
(Today, Pets, Care, Fostering, Account); it does not replace the workspace
switcher or duplicate the notification centre. Do not add a generic Home
control or unscoped tab strip elsewhere.
