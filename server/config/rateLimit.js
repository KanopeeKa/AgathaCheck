 ...
 *
 * The limiter is skipped under `NODE_ENV=test` so the Jest suite is not
 * throttled, unless a test opts in by setting `AUTH_RATE_LIMIT_TEST=1` (used by
 * the dedicated rate-limit test). Mirror any behavior change in the Dart
 * server's `lib/rate_limit.dart`.
 */
...
export function createAuthLimiter() {
  ...
  return rateLimit({
    ...
    legacyHeaders: false,
    skip: () =>
      process.env.NODE_ENV === 'test' && process.env.AUTH_RATE_LIMIT_TEST !== '1',
