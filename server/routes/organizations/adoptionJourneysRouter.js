...
import {
  ...
  completeVisitAndStartAdoptionJourney,
...
export function registerAdoptionJourneysRoutes(router, pool) {
  ...
  router.post('/:orgId/placements/:id/adoption-path/complete-visit-and-start', async (req, res) => {
    const userId = extractUserId(req);
    if (!userId) return res.status(401).json({ error: 'Unauthorized' });
    const { orgId, id: placementId } = req.params;
    const data = req.body || {};
    const visitId = (data.visit_id || data.visitId || '').trim() || null;
    const adoptionConditions = (data.adoption_conditions || data.adoptionConditions || '').trim();
    ...
    try {
      ...
      const result = await completeVisitAndStartAdoptionJourney(pool, {
        placement,
        orgId,
        visitId,
        adoptionConditions,
