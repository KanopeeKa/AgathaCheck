...
void registerOrgPetsRoutes(Router router, Pool pool) {
  router.get('/<id>/pets', (Request request, String id) async {
    ...
    try {
      if (!isActiveOrgMember(await getOrgMemberRole(pool, id, userId)))
        return orgForbidden();
      final results = await pool.execute(
        Sql.named(
            'SELECT * FROM pets WHERE organization_id = @orgId ORDER BY created_at'),
        parameters: {'orgId': id},
      );
  ...
  router.get('/<id>/archived', (Request request, String id) async {
    ...
    try {
      ...
      final results = await pool.execute(
        Sql.named(
            'SELECT * FROM archived_pets WHERE organization_id = @orgId ORDER BY created_at DESC'),
        parameters: {'orgId': id},
      );
