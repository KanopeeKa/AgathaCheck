  ...
  final orgUuid = Uuid();
    ...
    router.post('/<id>/invite', (Request request, String id) async {
      ...
      try {
        ...
        if (!canAssignOrgRole(actorRole, role)) {
          return Response(403, body: jsonEncode({'error': 'Forbidden'}), headers: orgJsonHeaders);
        }
    ...
    router.get('/<orgId>/people/<kind>/<personId>', (
      ...
      if (kind != 'member' && kind != 'external') {
        return Response(400, body: jsonEncode({'error': 'Invalid person kind'}), headers: orgJsonHeaders);
      }
      try {
        if (!await requireOrgAdmin(pool, orgId, userId)) return orgForbidden();
        final detail = await getOrgPersonDetail(pool, orgId, kind, personId);
        if (detail == null) {
          return Response.notFound(jsonEncode({'error': 'Person not found'}), headers: orgJsonHeaders);
        }
    ...
    router.put('/<orgId>/people/<kind>/<personId>/contact', (
      ...
      try {
        ...
        return Response(400, body: jsonEncode({'error': e.message}), headers: orgJsonHeaders);
      } catch (e) {
        return Response.internalServerError(body: jsonEncode({'error': publicError(e)}), headers: orgJsonHeaders);
      }
    });
      ...
      try {
        ...
        final results = await pool.execute(
          Sql.named('''
            ...
            RETURNING *
          '''),
          parameters: {'role': role, 'orgId': orgId, 'memberUserId': memberUserId},
        );
        if (results.isEmpty) {
          return Response.notFound(jsonEncode({'error': 'Member not found'}), headers: orgJsonHeaders);
        }
    ...
    router.delete('/<orgId>/members/me', (Request request, String orgId) async {
      ...
      try {
        ...
        return Response.ok(jsonEncode({'message': 'Left organization'}), headers: orgJsonHeaders);
      } catch (e) {
        return Response.internalServerError(body: jsonEncode({'error': publicError(e)}), headers: orgJsonHeaders);
      }
    });
    ...
    router.delete('/<orgId>/members/<memberUserId>', (
      ...
      try {
        if (!await requireOrgAdmin(pool, orgId, userId)) return orgForbidden();
        await pool.execute(
          Sql.named('DELETE FROM organization_users WHERE organization_id = @orgId AND user_id = @memberUserId'),
          parameters: {'orgId': orgId, 'memberUserId': memberUserId},
        );
        return Response.ok(jsonEncode({'message': 'Member removed'}), headers: orgJsonHeaders);
      } catch (e) {
        return Response.internalServerError(body: jsonEncode({'error': publicError(e)}), headers: orgJsonHeaders);
      }
    });
