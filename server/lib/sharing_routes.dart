...
Router sharingRoutes(Pool pool) {
  ...
  router.get('/pending', (Request request) async {
    ...
    try {
      final results = await pool.execute(
        Sql.named(
            "SELECT pa.*, p.name as pet_name FROM pet_access pa JOIN pets p ON p.id = pa.pet_id WHERE pa.user_id = @userId AND pa.role = 'pending_shared'"),
  ...
  router.post('/pending/<petId>/accept', (Request request, String petId) async {
    ...
    try {
      await pool.execute(
        Sql.named(
            "UPDATE pet_access SET role = 'shared' WHERE pet_id = @petId AND user_id = @userId AND role = 'pending_shared'"),
        parameters: {'petId': petId, 'userId': userId},
      );
      return Response.ok(jsonEncode({'message': 'Share accepted'}),
          headers: {'Content-Type': 'application/json'});
    } catch (e) {
      return Response.internalServerError(
          body: jsonEncode({'error': publicError(e)}));
  ...
  router.post('/pending/<petId>/decline',
      (Request request, String petId) async {
    final userId = _extractUserId(request);
    if (userId == null)
      return Response(401, body: jsonEncode({'error': 'Unauthorized'}));
    try {
      await pool.execute(
        Sql.named(
            "DELETE FROM pet_access WHERE pet_id = @petId AND user_id = @userId AND role = 'pending_shared'"),
        parameters: {'petId': petId, 'userId': userId},
      );
      return Response.ok(jsonEncode({'message': 'Share declined'}),
          headers: {'Content-Type': 'application/json'});
    } catch (e) {
      return Response.internalServerError(
          body: jsonEncode({'error': publicError(e)}));
  ...
  router.put('/<petId>/hide', (Request request, String petId) async {
    ...
    try {
      ...
      await pool.execute(
        Sql.named(
            'UPDATE pet_access SET hidden = @hidden WHERE pet_id = @petId AND user_id = @userId'),
  ...
  router.get('/hidden', (Request request) async {
    ...
    try {
      final results = await pool.execute(
        Sql.named(
            'SELECT pa.*, p.name as pet_name FROM pet_access pa JOIN pets p ON p.id = pa.pet_id WHERE pa.user_id = @userId AND pa.hidden = true'),
        parameters: {'userId': userId},
      );
      final list = results.map((r) => r.toColumnMap()).toList();
      return Response.ok(jsonEncode(list),
