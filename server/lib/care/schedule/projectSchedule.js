/**
 * Read-only care schedule projection for a date window.
 *
...
export const CERTAINTY_COMPLETE = 'complete';
export const CERTAINTY_CONDITIONAL_ON_FUTURE_COMPLETION = 'conditional_on_future_completion';
 ...
 */
function certaintyForItem(entry, source) {
  if (source === 'materialised') {
    return CERTAINTY_COMPLETE;
  }
  const anchor = entry.recurrence_anchor || 'from_completion';
  return anchor === 'from_completion'
...
function buildItem(entry, dateIso, time, source, status, occurrenceId = null) {
  return {
    ...
    status,
    source,
    certainty: certaintyForItem(entry, source),
    name: entry.name || '',
    type: entry.type,
 ...
 * @param {string} todayIso
 */
export function projectSchedule(entries, occurrencesByEntryId, startsOn, endsOn, todayIso) {
  const allItems = [];
...
/**
 * Backward-compatible alias for Care Context callers.
 *
 * @param {object[]} entries
 * @param {Map<string, object[]>} occurrencesByEntryId
 * @param {string} startsOn
