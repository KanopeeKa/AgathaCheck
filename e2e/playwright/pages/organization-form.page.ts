...
export class OrganizationFormPage {
  ...
  async selectType(type: 'Professional' | 'Charity'): Promise<void> {
    await selectDropdownOption(this.page, 'Organization Type', type);
  }
  ...
  async createOrganization(
    ...
    await this.save();
    await this.page.getByText('Organization created').waitFor({ timeout: 30_000 });
  }
  ...
  async updateBio(bio: string): Promise<void> {
    ...
    await this.save();
    await this.page.getByText('Organization updated').waitFor({ timeout: 30_000 });
  }
}
