...
import { enableFlutterAccessibility, refreshFlutterAccessibility, waitForFlutterRoutePattern, escapeRegExp, semanticsByName, toggleOrgPetsRefinementFilter } from '../support/flutter';
...
test.describe('Organisation pet filters', () => {
  ...
  test('Name filter narrows pets on the All tab', async ({ page }) => {
    ...
    await toggleOrgPetsRefinementFilter(page, 'Name');
  ...
  test('Shadow filter shows adopted shadow pets on the All tab', async ({ page }) => {
    ...
    await openOrgPetsScreen(page, baseURL, alice, org, 'Rescue Hearts');
    await selectOrgPetsTab(page, 'All');
    await toggleOrgPetsRefinementFilter(page, 'Shadow');
