...
description: Pet create/update accept organization_id from the request body; the backend must verify the caller belongs to that org before persisting.
---
...
from the request body. Before persisting, the backend verifies the authenticated
...
(org, shared-pet, etc.), validate the relationship server-side in the Node
(`server/routes/`) backend. Match the truthy/empty-string guard semantics (Node:
`if (orgId && ...)`).
