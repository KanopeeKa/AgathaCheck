...
- **Localization (EN/FR)**: Full English/French localization via Flutter's `intl` system. ARB files at `flutter_app/lib/l10n/app_en.arb` and `app_fr.arb` (~470+ keys). Generated `AppLocalizations` class imported as `import '../../../../l10n/app_localizations.dart';` (relative path). Locale persisted in SharedPreferences key `app_locale` via `LocaleNotifier` in `core/providers/locale_provider.dart`. Language selector in My Details screen. User locale also synced to server via `users.locale` column and included in `PUT /api/auth/me` requests. Server-side notification messages are locale-aware via `_t()` translation helper with `_translations` map (overdue, reminder, completed, memorial, CRUD notifications). PDF services (`pet_report_service.dart`, `events_pdf_service.dart`) accept `AppLocalizations l` parameter for localized report generation. Species values remain English internally; display text uses `_localizedSpecies()` helper.
