...
- **Health Tracking**: Comprehensive health entry management (medications, preventives, vaccines, other) with scheduling, photo attachments, and a tabbed dashboard view ("Events" page with tabs: All, Medications, Preventives, Vaccines, Other). Compact card design with pet photo+name strip on left, frequency badge, and "Done [date]" for completed entries. Delete moved to edit form. Data stored in PostgreSQL.
