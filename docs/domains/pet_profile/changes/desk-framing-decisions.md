...
| **D-desk-5** | **Open canvas default.** Home preview sections do not wrap content in tinted `GuardianDeskSectionCard` shells. Local cards/rows provide grouping. **Exceptions:** Care preview uses `petCareLight` block surface (matches pet-detail care preview); Fostering org tint may remain. Care Team stays open canvas with per-row `CareTeamCard` surfaces. | locked |
