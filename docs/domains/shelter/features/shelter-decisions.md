---
title: Shelter decisions
owner: Documentation Team
audience: both
status: active
last_updated: 2026-08-23
tags: [shelter, decisions]
domain: shelter
feature_id: shelter-decisions
---

# Shelter — locked decisions

Product decisions for organisation roles, discoverability, presentation, and v2–v4 delivery (D12–D16, D20–D31, D-v2-*, D-v3-*, D-v4-*). Other docs reference these IDs instead of restating rationale.
...
| **D13** | Working shape (draft): wire roles narrow to `associate \| foster \| admin \| super_admin` (4, reusing the existing `foster` org-membership flag which is already orthogonal to fostering *approval* per G0 §4.3). "Foster Admin / Pet Admin / Team Admin" from the brief become **named permission-key bundle presets** a Super Admin can apply to an `admin`-role member — built on G0's existing additive permission-key catalog ([g0-contract-pack.md](/docs/domains/fostering/features/g0-contract-pack.md) §7), not a parallel system. New `organization_permissions` override table (user_id, org_id, permission_key, granted_by, granted_at, revoked_at) carries both bundle-derived and individually-granted keys. | tbd | Phase 3 |
...
| **D16** | Every permission grant/revoke and role change is written to the existing `audit_events` table (`docs/ops/observability.md`) with new `event_type` values — fine-grained audit is non-negotiable. No new audit infrastructure; reuse existing retention tiers. | locked | Phase 3 |
...
Source of truth: [organisation-ux-v3-delivery-plan.md](../changes/organisation-ux-v3-delivery-plan.md). Execute-plan: `organisation-ux-v3-badd`.
...
Source of truth: [organisation-people-permissions-v4-delivery-plan.md](../changes/organisation-people-permissions-v4-delivery-plan.md).
...
- Locked brief: [shelter-dashboard-brief.md](shelter-dashboard-brief.md)
- Account routing (D27): [navigation-decisions.md](/docs/domains/navigation/features/navigation-decisions.md)
