...
## 13. Settled decisions (review 2026-09-21)

| Topic | Decision |
|-------|----------|
| Backfill `care_planning` | Conservative: occurrences/history signals; default ambiguous → `planned` (§5.4) |
| UI label for importance | **Priority** |
| `filter_group` in taxonomy | **Yes, v1** (§4.2) |
| Global “Record care” FAB | **Defer** past v1; pet-profile `?planning=unplanned` only |
| Health-issue prompt scope | **Vet-setting unplanned only** (§10.1) |
| Dental at home | Same `dental` family + `care_setting=home` |
| Client-sent `type` | **Reject Phase B**; Flutter cutover Phase C |
| `unplanned` + recurring | **400** |

### 13.1 Remaining open items

1. **Health issue from planned completion** — prompt families: all vet-setting completions, or only `wellness_review` / `dental` / `vaccination`?
2. **Create-at-completion detection** for backfill step 5 — confirm via `created_at ≈ completed_on` same day **and** zero occurrences (may still miss same-day planned+complete).
