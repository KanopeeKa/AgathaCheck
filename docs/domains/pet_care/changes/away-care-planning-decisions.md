...
**Status: proposed.** Product answers were given in chat on 2026-09-23 (summarised in the delivery plan §1.3). Each decision below becomes **Frozen** once reviewed in ACP-DOC-0. Decisions marked *amends* change a frozen decision and must be read together with it.
...
3. The corpus case `from-completion-overdue-before-window` (expects 0 items) is **replaced**, not deleted. It must expect the open occurrence to be surfaced. A new case covers the vanishing `from_due_date` entry.
...
- **Footnote copy (proposed):** "Estimated dates assume each care is done on its due date." This replaces the originally drafted wording ("based on the last done timing"), which described the calculation incorrectly.
...
|---|---|---|
| `fixed` | Never suggested for moving | `care_source ∈ {vet_instruction, treatment_schedule}` **or** `care_family = medication` |
...
Numbers are **proposed defaults**, to be confirmed in ACP-DOC-0 (see delivery plan §9).
...
3. The route returns `warnings[]` (non-blocking): `outside_flexibility` (D-ACP-006) and `interval_changed` (`{ previous_gap_days, usual_gap_days }`). The client renders these; the server stays the source of truth.
