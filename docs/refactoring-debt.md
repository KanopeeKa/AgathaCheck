...
|---|-------|----------|----------------|
| 1 | `health_entry_form_controller` | **Incremental wiring** (approved to proceed in a follow-up pass) | Move form state + submit/photo queue into `StateNotifier` first; keep screen as layout-only. Do **not** big-bang migrate 975 lines at once. |
...
| Health tracking | Partial | Widget extraction done; controller wiring next |
...
| `health_entry_form_controller.dart` | P2 | Incremental wiring approved. Phase 1: form state + submit + photos. Phase 2: recurrence/validation. |
