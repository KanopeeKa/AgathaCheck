...
**Agent policy:** `cursor/*` PRs may edit `.github/workflows/` (see `agent-safety-lib.js`); update `ci-gate` / `assert-ci-gate.sh` / `docs/pipelines/ci-cd-gates.md` in the same PR when adding blocking jobs.
