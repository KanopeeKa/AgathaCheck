...
**Optional (visible, not required individually):** `flutter-test-{pet-core,pet-screens,pet-widgets,health,org,rest-a,rest-b,experience} / Flutter tests (<shard>)` —
...
**Optional shard checks** (`flutter-test-{pet-core,pet-screens,pet-widgets,health,org,rest-a,rest-b,experience} / Flutter tests (<shard>)`) need
...
| `flutter-test-* / Flutter tests (<shard>)` | `_reusable-flutter-test-shard.yml` | domain test shards (pet-core, pet-screens, pet-widgets, health, org, rest-a, rest-b, experience) with per-shard coverage |
