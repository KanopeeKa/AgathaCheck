...
```sh
python3 /tmp/architecture-metrics.py --repo "$(git rev-parse --show-toplevel)" --output /tmp/architecture-metrics.md
```


## Appendix E — Measurement implementation

Save this block to /tmp/architecture-metrics.py and run the command in Appendix D from the repository root. The script writes only the requested output report. Measurements refer to the reviewed baseline, not future code.
