...
| `UAT full E2E (localhost)` (`uat-e2e-full`) | **Yes** | Full Playwright on localhost stack (4 parallel shards) |
| `Prod ready` (`prod-ready`) | **Yes** (aggregate) | Required for PROD environment gate |

**Parallelism:** `uat-e2e-full` runs four Playwright shards in parallel (each with its own Postgres + server) after `build-web` completes; shards still overlap with `deploy` FTP work. All four gates must pass for `prod-ready`.
