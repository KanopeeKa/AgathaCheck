...
**Parallelism:** `uat-e2e-full` runs six Playwright shards in parallel (each with its own Postgres + server) after `build-web` completes; shards still overlap with `deploy` FTP work. All four gates must pass for `prod-ready`.
