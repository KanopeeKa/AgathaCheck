...
enum FosterOnboardingStepState {
  ...
  static FosterOnboardingStepState fromWire(String? value) {
    switch (value) {
      case 'complete':
        return FosterOnboardingStepState.complete;
      case 'current':
        return FosterOnboardingStepState.current;
      case 'issue':
        return FosterOnboardingStepState.issue;
      default:
        return FosterOnboardingStepState.notStarted;
    }
  }
...
class FosterOnboardingStep {
  ...
  factory FosterOnboardingStep.fromJson(Map<String, dynamic> json) =>
...
class FosterOnboardingStatus {
  ...
  factory FosterOnboardingStatus.fromJson(Map<String, dynamic> json) =>
      FosterOnboardingStatus(
        resourceId: json['resource_id']?.toString() ?? '',
        steps: (json['steps'] as List<dynamic>? ?? const [])
            .whereType<Map>()
            .map(
              (e) =>
                  FosterOnboardingStep.fromJson(Map<String, dynamic>.from(e)),
            )
            .toList(),
      );
}
