...
class PlannedAbsenceHubPartition {
  ...
  static ({List<PlannedAbsence> upcoming, List<PlannedAbsence> past}) partition(List<PlannedAbsence> absences, {DateTime? today}) {
    final todayIso = PlannedAbsenceDateRules.startsOnWire(today ?? PlannedAbsenceDateRules.todayCalendar());
    ...
    for (final absence in absences) {
      ...
      if (absence.endsOn.compareTo(todayIso) >= 0) upcoming.add(absence); else past.add(absence);
    }
