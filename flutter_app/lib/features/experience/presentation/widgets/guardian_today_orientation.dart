...
class GuardianTodayOrientation extends StatelessWidget {
  const GuardianTodayOrientation({
    super.key,
    required this.state,
    this.summary,
    this.onRetry,
  ...
  final GuardianTodayScreenState state;
  final GuardianTodayCareSummary? summary;
  ...
  Widget build(BuildContext context) {
    ...
    return Semantics(
      key: const Key('guardian_today_orientation'),
      container: true,
      header: true,
      label: semanticLabel,
      ...
      child: DecoratedBox(
        ...
        child: Padding(
          ...
          child: Column(
            key: Key('guardian_today_orientation_${resolvedState.name}'),
  ...
  GuardianTodayScreenState get _resolvedState {
    final summaryRequired =
        state == GuardianTodayScreenState.attention ||
        state == GuardianTodayScreenState.allClear;
    return summaryRequired && summary == null
        ? GuardianTodayScreenState.partial
        : state;
  }

  String _semanticLabel(AppLocalizations l, GuardianTodayScreenState state) {
    switch (state) {
      case GuardianTodayScreenState.attention:
        return _attentionLabel(l);
      case GuardianTodayScreenState.allClear:
        return '${l.today}: ${l.todayAllClear}';
      case GuardianTodayScreenState.firstUse:
        return '${l.today}: ${l.guardianOnboardingWelcomeBody}';
      case GuardianTodayScreenState.loading:
        return l.today;
      case GuardianTodayScreenState.partial:
        return '${l.today}: ${l.careLoadError}';
      case GuardianTodayScreenState.error:
  ...
  Widget _body(
    ...
    AppLocalizations l,
    GuardianTodayScreenState state,
  ) {
    final theme = Theme.of(context);
    switch (state) {
      case GuardianTodayScreenState.attention:
