...
void main() {
  ...
  PetCareTodayCareSummary summary({
    ...
    return PetCareTodayCareSummary.forPets(
      entries: entries,
      pets: const [pet],
      now: now,
    );
  ...
  Widget buildSubject({
    required PetCareTodayScreenState state,
  ...
  testWidgets('attention presents every urgency count once in semantics', (
    ...
    final node = tester.getSemantics(
      find.byKey(const Key('pet_care_today_orientation')),
    );
    expect(
      node.label,
  ...
  testWidgets(
    ...
    (tester) async {
      for (final state in PetCareTodayScreenState.values) {
        await tester.pumpWidget(
          buildSubject(
            state: state,
        ...
        expect(
          find.byKey(Key('pet_care_today_orientation_${state.name}')),
          findsOneWidget,
        );
  ...
  testWidgets(
    ...
    (tester) async {
      await tester.pumpWidget(
        buildSubject(state: PetCareTodayScreenState.attention),
      );

      expect(
        find.byKey(const Key('pet_care_today_orientation_partial')),
        findsOneWidget,
      );
  ...
  testWidgets('retry has an accessible 48dp target and invokes its callback', (
    ...
    await tester.pumpWidget(
      buildSubject(
        state: PetCareTodayScreenState.error,
        onRetry: () => retries++,
      ),
    );
  ...
  testWidgets('wraps long French copy at 200% text scale without overflow', (
    ...
    expect(find.byKey(const Key('pet_care_today_orientation')), findsOneWidget);
    expect(tester.takeException(), isNull);
  });
