...
void main() {
  ...
  group('getEntries', () {
    test('returns entries from data source', () async {
      when(
        mockDataSource.getEntries(petId: null, type: null),
    ...
    test('passes filter parameters to data source', () async {
      when(
        mockDataSource.getEntries(petId: 'pet-1', type: 'medication'),
      ).thenAnswer((_) async => [testModel]);
  ...
  group('getEntry', () {
    test('returns entry from data source', () async {
      when(
        mockDataSource.getEntry('test-1'),
      ).thenAnswer((_) async => testModel);
    ...
    test('returns null when not found', () async {
      when(mockDataSource.getEntry('missing')).thenAnswer((_) async => null);
  ...
  group('createEntry', () {
    test('delegates to data source', () async {
      when(mockDataSource.createEntry(any)).thenAnswer((_) async => testModel);
  ...
  group('updateEntry', () {
    test('delegates to data source', () async {
      when(mockDataSource.updateEntry(any)).thenAnswer((_) async => testModel);
  ...
  group('deleteEntry', () {
    test('delegates to data source', () async {
      when(mockDataSource.deleteEntry('test-1')).thenAnswer((_) async {});
  ...
  group('getHistory', () {
    test('returns history from data source', () async {
      ...
      when(
        mockDataSource.getHistory('test-1'),
  ...
  group('exportCsv', () {
    test('returns CSV string from data source', () async {
      when(
        mockDataSource.exportCsv(petId: null),
