...
void main() {
  testWidgets('placement section shows not-in-foster actions', (tester) async {
    ...
    expect(find.byKey(const Key('start_foster_placement_button')), findsOneWidget);
    expect(find.byKey(const Key('direct_adopt_button')), findsOneWidget);
  });

  testWidgets('start placement shows snackbar when no member foster parents exist', (tester) async {
    await tester.pumpWidget(
      ProviderScope(
        overrides: [
          authProvider.overrideWith((ref) => FakeAuthNotifier()),
  ...
  testWidgets('start placement opens foster parent picker for member fosters', (tester) async {
    await tester.pumpWidget(
      ProviderScope(
        overrides: [
          authProvider.overrideWith((ref) => FakeAuthNotifier()),
