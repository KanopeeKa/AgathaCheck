...
for filepath in $(find "$REPO_ROOT/docs" -name "*.md" -type f 2>/dev/null); do
  # Check for valid YAML frontmatter: --- at start, followed by metadata, followed by ---
  if ! (grep -qE '^---$' "$filepath" && \
        head -10 "$filepath" | grep -qE '^(title|owner|audience|status|last_updated|tags):'); then
...
for filepath in "$REPO_ROOT"/*.md; do
  if [[ -f "$filepath" ]] && ! (grep -qE '^---$' "$filepath" && \
        head -10 "$filepath" | grep -qE '^(title|owner|audience|status|last_updated|tags):'); then
    echo "  Missing metadata header: $filepath"
...
ARCHIVED_FILES=(
  "docs/design/navigation-v2.md"
  "docs/experience-split-plan.md"
  "docs/quality/review-2026-07-08.md"
)

ARCHIVED_REFERENCES=0
for old_path in "${ARCHIVED_FILES[@]}"; do
