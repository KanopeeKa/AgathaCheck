...
curl_api() {
  ...
  if ! response="$(curl -sfSm 45 -H "$AUTH" "$url")"; then
    echo "::error::o2switch SshWhitelist API request failed: $url" >&2
    exit 1
  fi
...
case "$ACTION" in
  remove_all)
    echo "Clearing o2switch SSH whitelist slots (CI reset)..."
    curl_api "${BASE}/remove_all"
    ;;
  add)
    [ -n "$RUNNER_IP" ] || { echo "::error::RUNNER_IP required for add" >&2; exit 1; }
    echo "Whitelisting runner IP ${RUNNER_IP} on port ${PORT}..."
    curl_api "${BASE}/add?address=${RUNNER_IP}&port=${PORT}"
    ;;
  list)
    curl_api "${BASE}/list"
    ;;
